...
def test_llama_pipeline():
    ...
    assert "examples" in examples and len(examples["examples"]) > 0, "No examples found for RSI"
    ...
    assert "strategy" in strategy and len(strategy["strategy"]) > 50, "Generated strategy too short or missing"
    ...
    assert "summary" in summary and len(summary["summary"]) > 20, "Generated summary too short or missing"
...
if __name__ == "__main__":
    ...
    #    or `python tests/test_llama_integration.py` (if it contains direct executable logic)
    test_llama_pipeline()
