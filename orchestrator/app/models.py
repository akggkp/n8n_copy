...
class ProvenStrategy(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    media_item_id = db.Column(db.Integer, db.ForeignKey("processed_videos.id", ondelete="CASCADE")) # Link to ProcessedVideo.id
    ...
    # Status
    status = db.Column(db.String(50), default='promoted')  # promoted, active, retired
...
class MlStrategy(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    media_item_id = db.Column(db.Integer, db.ForeignKey("media_items.id", ondelete="CASCADE"), unique=True, nullable=False) # Changed from video_id
...
class MediaItem(db.Model):
    ...
    # Relationships
    transcripts = db.relationship("Transcript", back_populates="media_item", cascade="all, delete-orphan")
    keyword_hits = db.relationship("KeywordHit", back_populates="media_item", cascade="all, delete-orphan")
    frames = db.relationship("Frame", back_populates="media_item", cascade="all, delete-orphan")
    clips = db.relationship("Clip", back_populates="media_item", cascade="all, delete-orphan")
    embeddings = db.relationship("Embedding", back_populates="media_item", cascade="all, delete-orphan")

class Transcript(db.Model):
    __tablename__ = "transcripts"
    
    id = db.Column(db.Integer, primary_key=True)
    media_item_id = db.Column(db.Integer, db.ForeignKey("media_items.id", ondelete="CASCADE"))
...
class Clip(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    media_item_id = db.Column(db.Integer, db.ForeignKey("media_items.id", ondelete="CASCADE"))
    keyword_hit_id = db.Column(db.Integer, db.ForeignKey("keyword_hits.id", ondelete="SET NULL"))
...
class Embedding(db.Model):
    ...
    embedding_vector = db.Column(JSONB) # Using JSONB for ARRAY(Float) in Flask-SQLAlchemy
