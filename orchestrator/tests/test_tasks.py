...
def test_validate_video_file_not_found(mocker):
    ...
    mocker.patch('app.tasks.validate_video.retry',
                 side_effect=Exception("Task retry triggered"))
...
def test_validate_video_already_processed(mocker):
    ...
    with patch('app.tasks.db_session') as mock_db_session:
        mock_session = mock_db_session().__enter__()
        mock_session.query.return_value.filter_by.return_value.first.return_value = ProcessedVideo(
            video_id=video_id)
