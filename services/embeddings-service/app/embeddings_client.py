...
class EmbeddingsClient:
    ...
    def _load_indices(self):
        ...
        try:
            ...
            if transcript_index_path.exists():
                self.transcript_index = faiss.read_index(str(transcript_index_path))
                with open(self.faiss_index_dir / "transcript_metadata.pkl", "rb") as f:
                    self.transcript_metadata = pickle.load(f)
                logger.info(f"Loaded transcript index with {self.transcript_index.ntotal} vectors")
            else:
            ...
            if clip_index_path.exists():
                ...
                logger.info(f"Loaded clip index with {self.clip_index.ntotal} vectors")
            else:
    ...
    def add_transcript_embeddings(
        ...
        try:
            ...
            logger.info(f"Added {len(embeddings)} transcript embeddings for media_item {media_item_id}")
    ...
    def add_frame_embeddings(
        ...
        try:
            ...
            # In production, use CLIP or similar vision model for actual image embeddings
            texts = [f"Frame at {f['timestamp']}s from video {media_item_id}" for f in frames]
            embeddings = self.encode_text(texts)
            ...
            logger.info(f"Added {len(embeddings)} frame embeddings for media_item {media_item_id}")
            return len(embeddings)
    ...
    def add_clip_embeddings(
        ...
        try:
            ...
            texts = [f"{clip.get('keyword', '')} {clip.get('context', '')}" for clip in clips]
            embeddings = self.encode_text(texts)
            ...
            logger.info(f"Added {len(embeddings)} clip embeddings for media_item {media_item_id}")
            return len(embeddings)
    ...
    def search_transcripts(
        ...
        try:
            ...
            logger.info(f"Transcript search for '{query}' returned {len(results)} results")
            return results
    ...
    def search_frames(
        ...
        try:
            ...
            query_embedding = self.encode_text([query])[0:1]
            distances, indices = self.frame_index.search(query_embedding, top_k)
            ...
            logger.info(f"Frame search for '{query}' returned {len(results)} results")
            return results
    ...
    def search_clips(
        ...
        try:
            ...
            logger.info(f"Clip search for '{query}' returned {len(results)} results")
            return results
    ...
    def save_indices(self):
        ...
        try:
            ...
            if self.transcript_index:
                faiss.write_index(self.transcript_index, str(self.faiss_index_dir / "transcript_index.faiss"))
                with open(self.faiss_index_dir / "transcript_metadata.pkl", "wb") as f:
                    pickle.dump(self.transcript_metadata, f)
            
            # Save frame index
            if self.frame_index:
                faiss.write_index(self.frame_index, str(self.faiss_index_dir / "frame_index.faiss"))
                with open(self.faiss_index_dir / "frame_metadata.pkl", "wb") as f:
                    pickle.dump(self.frame_metadata, f)
            
            # Save clip index
            if self.clip_index:
                faiss.write_index(self.clip_index, str(self.faiss_index_dir / "clip_index.faiss"))
    ...
    def get_stats(self) -> Dict:
        ...
        return {
            ...
            'total_vectors': (self.transcript_index.ntotal if self.transcript_index else 0) +
