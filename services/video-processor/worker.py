...
from keyword_detector import detect_keywords_from_whisper_result
...
# Configuration
DELETE_VIDEO_AFTER_PROCESSING = os.getenv(
...
# Base.metadata.create_all(engine)  # This should be handled by a separate
# migration script (e.g., using Alembic)
...
class MemoryManager:
    ...
    def cleanup(self):
        ...
        logger.info(
            f"Memory cleanup: {mem_before:.1f}MB → {mem_after:.1f}MB (freed {freed:.1f}MB)")
        self.last_cleanup = datetime.now()
...
class VideoProcessorCascade:
    ...
    def extract_audio_and_transcribe(self, video_file):
        ...
        try:
            ...
            if WHISPER_CASCADE:
                ...
                if confidence < 0.5:
                    logger.info(
                        "Confidence low, cascading to larger Whisper model...")
                    if self.whisper_cascade_model is None:
                        self.whisper_cascade_model = whisper.load_model(
                            WHISPER_CASCADE_MODEL)
            ...
            logger.info(
                f"Transcription completed: {len(text)} characters, {len(result.get('segments', []))} segments")
    ...
    def delete_video_file(self, video_file):
        ...
        try:
            if os.path.exists(video_file):
                ...
                logger.info(
                    f"Deleted video file: {video_file} ({file_size:.1f}MB)")
        except Exception as e:
    ...
    def process_video(self, video_id, file_path):
        ...
        try:
            ...
            transcription_result = self.extract_audio_and_transcribe(
                video_file)
            ...
            logger.info(
                f"Found {len(keyword_hits)} keyword hits: {keyword_stats.get('unique_keywords', 0)} unique keywords")
            ...
            with session_scope() as session:
                ...
                extended_stats['unique_keywords'] = keyword_stats.get(
                    'unique_keywords', 0)
                extended_stats['top_keywords'] = keyword_stats.get('top_keywords', [])[
                    :5]
                extended_stats['video_duration'] = video_duration
                extended_stats['transcript_segments_count'] = len(
