...
class MemoryCleaner:
    ...
    def cleanup(self) -> Dict[str, float]:
        ...
        logger.info(
            f"System: {sys_used:.1f}GB used, {sys_available:.1f}GB available")
        ...
        try:
            ...
            logger.info(
                f"Process memory freed: {freed:.1f} MB ({(freed/mem_before*100):.1f}%)")
            logger.info(
                f"System: {sys_used_after:.1f}GB used, {sys_available_after:.1f}GB available")
