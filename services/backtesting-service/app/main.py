    ...
    try:
        ...
        for i, bar in enumerate(price_data):
            ...
            if position is None:
                ...
                should_enter = evaluate_entry_rules(
                    entry_rules, bar, price_data[:i])
                ...
                if should_enter:
                    ...
                    logger.debug(
                        f"Enter at {bar['close']:.2f}, shares={shares:.4f}")
                ...
                should_exit = (
                    ...
                    evaluate_exit_rules(
                        exit_rules, bar, price_data[:i])  # Exit rules
                )
                ...
                if should_exit:
                    ...
                    logger.debug(
                        f"Exit at {exit_price:.2f}, PnL={pnl:.2f} ({pnl_pct:.2f}%)")
                    position = None
...
def calculate_performance_metrics(
    ...
    try:
        ...
        sharpe_ratio = np.mean(returns) / (np.std(returns) + 1e-10) * \
            np.sqrt(252) if len(returns) > 0 else 0.0
...
async def run_backtest(request: BacktestRequest):
    ...
    try:
        ...
        logger.info(
            f"Backtest {backtest_counter} completed for strategy {request.strategy_id}")
