...
class BacktestEngine:
    ...
    def run_simulation(self, data: pd.DataFrame,
        ...
        profit_factor = abs(
            ...
            losing_trades['pnl'].sum()) if len(losing_trades) > 0 and losing_trades['pnl'].sum() != 0 else 0

        # Calculate Sharpe Ratio (simplified)
        returns = trades_df['pnl'].pct_change().dropna()
        sharpe_ratio = returns.mean() / returns.std() * \
            np.sqrt(252) if len(returns) > 0 and returns.std() > 0 else 0
    ...
    def backtest_strategy(self,
        ...
        try:
            ...
            logger.info(
                f"Results: WR={metrics['win_rate']:.1f}%, PF={metrics['profit_factor']:.2f}, SR={metrics['sharpe_ratio']:.2f}")
