...
class CascadeAnalyzer:
        ...
        try:
            ...
            if result:
                return {
                    ...
                    'avg_frames_per_video': float(result[1]) if result[1] else 0,
                    'avg_detections_per_video': float(result[2]) if result[2] else 0,
                    'avg_processing_time': float(result[3]) if result[3] else 0
                }
    ...
    def print_report(self, hours: int = 24):
        ...
        if summary:
            ...
            print(f"Average frames per video: {summary['avg_frames_per_video']:.1f}")
            print(f"Average detections per video: {summary['avg_detections_per_video']:.1f}")
            print(f"Average processing time: {summary['avg_processing_time']:.1f}s")
